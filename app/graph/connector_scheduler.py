# Background scheduler that periodically syncs every tenant's connectors, so
# "Sync now" isn't the only way a connector's content stays current (see
# CLAUDE.md's v1 goal: "...without a human clicking sync"). Uses
# APScheduler's AsyncIOScheduler since this runs inside the same asyncio
# event loop uvicorn already owns, so no separate process or worker is
# needed at this scale. Started/stopped from app/main.py's lifespan.
#
...
# replica wins it for this tick actually runs, and the rest skip and try
# again next tick. See that module's docstring for why a Neo4j-backed
# lease (rather than moving this to a separate dedicated worker process)
# was the right-sized fix here.
import logging
import uuid
...
# One id per process, generated once at import time and stable for this
...
async def _renew_expiring_push_subscriptions(neo4j_client: Neo4jClient) -> None:
    ...
    _sync_all_connectors, not on its own separate schedule, since one
    ...
    for connector in connectors.list_connectors_with_push_subscriptions(repo=repo):
        ...
        try:
            ...
            # gone, etc.), so fall back to polling-only rather than
            # retrying a renewal that will keep failing every tick.
