...
async def refresh_access_token(refresh_token: str) -> str:
    ...
    tokens are only valid ~1 hour and a background sync may run far less
    often than that."""
    client_id, client_secret = _require_client_credentials()
    async with httpx.AsyncClient(timeout=15.0) as client:
        try:
...
async def revoke_token(token: str) -> None:
    ...
    settings), not just deleting our own pointer to it. Never raises --
    the connector deletion itself should still succeed even if Google's
    revoke endpoint is unreachable or the token's already invalid."""
